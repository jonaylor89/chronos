...
#>
function lint() {
    poetry run flake8 src\chronos tests
}
...
#>
function coverage() {
    poetry run coverage run --source src\chronos -m pytest
...
#>
function docs(){
    if (Test-Path ".\docs\chronos.rst") { Remove-Item ".\docs\chronos.rst" -Force }
    if (Test-Path ".\docs\modules.rst") { Remove-Item ".\docs\modules.rst" -Force }
    poetry run sphinx-apidoc -o docs\ src\chronos
